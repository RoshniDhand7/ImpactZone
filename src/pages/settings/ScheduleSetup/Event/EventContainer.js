...
const EventContainer = () => {
  ...
  const MappedServiceTemplate = (rowData) => {
    return <div>{rowData?.services?.length > 0 ? "Yes" : "No"}</div>;
  };

  const LocationTemplate = (rowData) => {
    return (
      <>
        <div className="flex">{rowData?.locationType?.name}</div>
      </>
    );
  };
  ...
  const serviceSelectHandle = (e) => {
    ...
    let newArray = array.filter((item) => {
      return !selectedArray.find((child) => {
        return item.level._id == child._id;
      });
    });
    let Update = selectedArray.filter((item) => {
    ...
    let dashArray = array.filter((item) => {
      return !newArray.find((child) => {
  ...
  const DeleteService = () => {
    ...
    let filterService = allAddEvent.services.filter((item, index) => {
      return index !== serviceDetailIndex;
    });
    let newOuterService = {
      ...allAddEvent,
  ...
  const changePosition = (value) => {
    ...
    if (value == "up" && serviceDetailIndex !== 0) {
      allData.services[serviceIndex].services.splice(
        serviceDetailIndex - 1,
        0,
      ...
      value == "down" &&
      serviceDetailIndex !== allData.services[serviceIndex].services.length - 1
    ) {
      allData.services[serviceIndex].services.splice(
        serviceDetailIndex + 1,
        0,
  ...
  const submitNewName = (next) => {
    let payload = {
      ...addEventData,
      name: newName,
      _id: null,
    };
    if (showAllFormErrors(payload, setAddEventData, required, initialData)) {
      dispatch(addEvents(payload)).then((data) => {
        if (data.success) {
          dispatch(getEvents());
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      let filteredDuration = durationOptions.filter((item) => {
        return !isEdit.durations.find((child) => {
          return item === child;
        });
      });
      let filteredcalendarOptions = calendarOptions.filter((item) => {
        return !isEdit.calendarDisplay.find((child) => {
          return item === child;
        });
      });
      let filteredcalendarPopOptions = calendarPopOptions.filter((item) => {
        return !isEdit.popupDisplay.find((child) => {
          return item === child;
        });
      });
      let filteredclubSource = clubSource.filter((item) => {
        return !isEdit.deployedClubs.find((child) => {
      ...
      setClubSource(filteredclubSource);
