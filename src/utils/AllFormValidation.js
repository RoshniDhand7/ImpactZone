...
const FormValidation = (name, value, data, required, initialData) => {
  ...
  switch (name) {
    ...
    case "password":
      ...
      case "zipCode":
        // console.log("lookup",zipcodes.lookupByName(data?.city,data?.state).find((item)=>{return item?.zip==value})?.zip==value,zipcodes?.lookupByName(data?.city,data?.state),value)
        if (name != "formErrors" && required.includes(name)) {
        if (equal(length(value))) {
          formErrors[name] = `${FirstletterUpperCase(name)} is required!`;
        } else if (zipcodes.lookupByName(data?.city,data?.state).find((item)=>{return item?.zip==value})?.zip!=value) {
          formErrors[
            name
