...
const AddMembers = () => {
    ...
    const prospectAgreement = allMembershipPlan?.filter((item) => item.isOneTimePlan)?.map((item) => ({ name: item.name, value: item._id }));
    const memberagreement = allMembershipPlan?.filter((item) => !item.isOneTimePlan)?.map((item) => ({ name: item.name, value: item._id }));
